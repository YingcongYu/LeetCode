...
class UndergroundSystem:
    ...
    def checkOut(self, id: int, stationName: str, t: int) -> None:
        start, time = self.customers.pop(id)

        if (start, stationName) in self.travel_time:
            self.travel_time[(start, stationName)][0] += t - time
            self.travel_time[(start, stationName)][1] += 1
        else:
            self.travel_time[(start, stationName)] = [t - time, 1]

    def getAverageTime(self, startStation: str, endStation: str) -> float:
        return self.travel_time[(startStation, endStation)][0] / self.travel_time[(startStation, endStation)][1]
